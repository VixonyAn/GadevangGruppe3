...
namespace GadevangGruppe3Razor.Services
{
    public class BrugerService : IBrugerService
    {
		...
		public async Task<List<Bruger>> FilterBrugerByMedlemskabsTypeAsync(MedlemskabsType filterMedlemskabsTypeCriteria)
		{
			List<Bruger> brugere = new List<Bruger>();
			using (SqlConnection connection = new SqlConnection(connectionString))
			{
				try
				{
					SqlCommand command = new SqlCommand(selectSql + " where Medlemskab = @Criteria", connection);
                    command.Parameters.AddWithValue("@Criteria", filterMedlemskabsTypeCriteria);
					await command.Connection.OpenAsync();
					SqlDataReader reader = await command.ExecuteReaderAsync();
					while (await reader.ReadAsync())
					{
		...
		public async Task<List<Bruger>> FilterBrugerByPositionAsync(Position filterPositionCriteria)
		{
			List<Bruger> brugere = new List<Bruger>();
			using (SqlConnection connection = new SqlConnection(connectionString))
			{
				...
				{
					SqlCommand command = new SqlCommand(selectSql + " where Medlemskab = @Criteria", connection);
					command.Parameters.AddWithValue("@Criteria", filterPositionCriteria);
					await command.Connection.OpenAsync();
					SqlDataReader reader = await command.ExecuteReaderAsync();
					while (await reader.ReadAsync())
					{
